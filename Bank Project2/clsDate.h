...
public:
	...
	static string GetSystemDateTimeString()
	{
		...
		string strDay = to_string(now->tm_mday);
		string strMonth = to_string(now->tm_mon + 1);
		string strYear = to_string(now->tm_year + 1900);

		string strHour = to_string(now->tm_hour);
		string strMinutes = to_string(now->tm_min);
		string strSeconds = to_string(now->tm_sec);

		DateTime += strDay + '/' + strMonth + '/' + strYear;
		DateTime += "  -  ";
		DateTime += strHour + ':' + strMinutes + ':' + strSeconds;
